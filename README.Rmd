...
Use the formula syntax to define data accessors that deck.gl uses to access the properties of the data object:
...
An object of class [sf](https://github.com/r-spatial/sf) is a `data.frame` with a geometry list-column.
Set the layer prop that fetches the geometry to the geometry column of your `sf` object:
...
The tooltip string is a so called "mustache" template in which variable names are identified
...
Controls are displayed as overlays on top of the map / deck.
Usually you can set the position and the style of the control.
...
The JavaScript library of deckgl uses [webpack](https://webpack.js.org/) as module bundler.
Therefore, you need [node.js](https://nodejs.org) to build the module.
